...
const send = async (template) => {
    ...
    if (!template || !template.to) {
        console.warn('[EMAIL] Invalid template. Skipping.');
        return false;
    }
...
const sendReportEmail = async (email, keyword, htmlContent) => {
    ...
    if (!email || !email.includes('@') || !email.includes('.')) {
        console.warn('[EMAIL] Invalid email for report:', email);
        return false;
    }
    ...
    try {
        ...
        const tempPath = path.join(__dirname, `../temp_products/report_${Date.now()}.pdf`);
        ...
        try {
            ...
            console.warn('[EMAIL] PDF creation failed, sending HTML only:', pdfErr.message);
        }
        ...
        const success = await send({
            ...
            html: `<div style="font-family:Arial,sans-serif;line-height:1.6;color:#333;max-width:600px;margin:0 auto;">
                <h2 style="color:#0D1B2A;">Your Report is Ready!</h2>
                <p>Attached is your comprehensive analysis for <strong>"${keyword}"</strong>.</p>
                <p><strong>Next Steps:</strong></p>
                <ul>
                    <li>Review the key findings in the report</li>
                    <li>Check out the <a href="${ctaUrl}" style="color:#F4A81D;">${ctaText}</a> for the complete blueprint</li>
                </ul>
                <p>Talk soon,<br/>Yokesh, Founder of NicheReport.ai</p>
                <hr style="border:none;border-top:1px solid #eee;margin:20px 0;"/>
                <p style="font-size:12px;color:#888;">Generated by NicheReport.ai</p>
            </div>`,
